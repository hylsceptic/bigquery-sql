truncate table `chaingraph-318604.eth_dm.eth_address_score`;
insert into `chaingraph-318604.eth_dm.eth_address_score`

-- create table `chaingraph-318604.eth_dm.eth_address_score`
-- PARTITION BY
...
(
    select distinct 
        ...
        0 as idx,
        1 as sub_weight
    from `chaingraph-318604.ethereum_tags.eth_address_tags`
    union all 
    select 
        address,
        plots,
        idx,
        sub_weight
    from `chaingraph-318604.ethereum_tags.generated_tags`
    union all 
    select 
        address,
        plots,
        idx,
        1 as sub_weight
    from `chaingraph-318604.ethereum_tags.eth_tags_view` 
),
...
-- select * from `chaingraph-318604.ethereum_tags.eth_tags_view`  where address is null
select
    *,
